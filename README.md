...
- **Connection Pooling (`ConnPool`):** Maintains an in-memory thread-safe connection pool of persistent backend TCP sockets, drastically reducing TCP handshake overhead and latency.
...
4. Proxy retrieves an idle TCP connection from `ConnPool` (or dials a fresh connection if empty).
...
7. Connection is returned to `ConnPool` for future reuse.
8. Background `healthCheck` goroutine pings backends on the interval specified in `config.yaml`.
...
- **Networking:** `net.Listen`, `net.Dial`, `bufio`, `io`
...
**Connection Pooling:** Built-in connection pooling (`ConnPool`) manages reusable persistent backend connections, avoiding repeated TCP setup and teardown overhead on sequential requests.
...
- [x] **Connection pooling** to backends (`ConnPool` implemented)
